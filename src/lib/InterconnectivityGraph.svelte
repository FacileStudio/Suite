<script lang="ts">
  ...
  const nodes = defs.map((name, i) => {
    ...
    return { name, x, y, lx, ly, anchor, i };
  });
...
<div bind:this={container} class="graph" class:active>
  <svg
    ...
    aria-label="Schéma des connexions entre les outils Facile via Antenne"
    role="img"
  >
    <defs>
    ...
    {#each nodes as n}
      <g class="app-node" style="--delay:{300 + n.i * 50}ms">
        <circle
          cx={n.x}
          cy={n.y}
          r="5"
    ...
    <circle
      ...
      stroke-width="1.5"
    />
    <g transform="translate({CX - 12}, {CY - 12}) scale(0.85)">
